...
likelihoodRatioTest <- function(expData1, expData2, spikeInScalingFactor) {
    ...
    chiHat1 <- rc1$chi; chiHat2 <- rc2$chi; maxItr <- 500; tor <- 1e-6
    ...
    emRes <- pmap(
        ...
        function(x, y, z, k, m, n) {
            tryCatch(
                mainEMH0(fkInt,
                    Xk1 = x, Xk2 = y, kmin, kmax, betaInt = z, chiHat = k,
                    chiHat1 = m, chiHat2 = n, maxItr, tor
                ),
                error = function(err) {
    ...
    betaTblIdx <-
        tibble(
            geneId = names(emHc), beta1 = map_dbl(emHc, "beta"),
            beta2 = map_dbl(emHt, "beta"), lfc = log2(beta2 / beta1),
            fkMean1 = map_dbl(emHc, "fkMean"),
            fkMean2 = map_dbl(emHt, "fkMean"), fkVar1 = map_dbl(emHc, "fkVar"),
