...
methods::setClass("transcription_rates_lrts",
                  slots = c(exp_data1="experiment_transcription_rates",
...
likelihood_ratio_test <- function(exp_data1, exp_data2, sc) {
    ...
    omega_tbl <-
    tibble(gene_id = exp_data1$gene_id,
            chi1 = exp_data1$chi,
    ...
    ggsave(file.path(result_dir, "chi_distribution.png"), plot = p,
        width = 4, height = 5)
    ...
    ggsave(file.path(result_dir, "chi_mean_vs_lfc.png"), plot = p,
        width = 6, height = 3)
    ...
    ggsave(file.path(result_dir, "beta_mean_vs_lfc.png"), plot = p,
        width = 6, height = 3)
    ...
    ggsave(file.path(result_dir, "gene_number_with_differential_rates.png"), plot = p,
        width = 6, height = 4)

    # mean and variance of pause sites in different beta categories
    beta_tbl <- beta_tbl %>%
    mutate(fk_std1 = fk_var1 ^ 0.5,
            fk_std2 = fk_var2 ^ 0.5) 
    ...
    ggsave(filename = file.path(result_dir, "mean_vs_std_of_k_pointdensity.png"), plot = p,
        width = 14, height = 5)

    # output tables
    write_csv(omega_tbl %>% select(gene_id, chi1, chi2, lfc, t_stats, padj), file = omega_out)
    write_csv(beta_tbl %>%
                select(gene_id, beta1, beta2, lfc,  t_stats, padj,
                    fk_mean1, fk_mean2, fk_var1, fk_var2), file = beta_out)
}
