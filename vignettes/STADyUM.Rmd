...
comparison_df <- data.frame(
  gene_id = c(exp_rates_control@rates$gene_id, exp_rates_treatment@rates$gene_id),
  beta_adp = c(exp_rates_control@rates$beta_adp, exp_rates_treatment@rates$beta_adp),
  condition = rep(c("Control", "Treatment"), each = nrow(exp_rates_control@rates))
)
...
ggplot(comparison_df, aes(x = condition, y = beta_adp, fill = condition)) +
  ...
  labs(title = "Comparison of Pause Release Rates",
       x = "Condition",
       y = "Beta (pause release rate)") +
  theme_minimal()

# Create a scatter plot of original vs adapted rates
ggplot(exp_rates_control@rates, aes(x = beta_org, y = beta_adp)) +
  geom_point(alpha = 0.6, color = "#1E88E5") +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  labs(title = "Original vs Adapted Pause Release Rates",
       x = "Original Beta",
       y = "Adapted Beta") +
  theme_minimal()

# Create a density plot of pause site positions
ggplot(exp_rates_control@rates, aes(x = fk_mean)) +
  geom_density(fill = "#1E88E5", alpha = 0.6) +
  labs(title = "Distribution of Pause Site Positions",
       x = "Mean Pause Site Position",
       y = "Density") +
  theme_minimal()

# Create a scatter plot of phi vs beta
ggplot(exp_rates_control@rates, aes(x = beta_adp, y = phi)) +
  geom_point(alpha = 0.6, color = "#1E88E5") +
  labs(title = "Relationship between Pause Release Rate and Landing Pad Occupancy",
